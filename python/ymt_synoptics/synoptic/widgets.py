...
class _QtSignal(Protocol):
    ...
    def __getitem__(self, signature: Union[type, str]) -> "_QtSignal":
        ...

    def connect(
            self,
            slot: Callable[[Optional[Union[int, str]]], None]) -> None:
        ...
...
class toggleCombo(QtWidgets.QComboBox):
    ...
    def __init__(self, parent: Optional[QtWidgets.QWidget] = None) -> None:
        ...
        self._connectCurrentIndexChanged()

    def _connectCurrentIndexChanged(self) -> None:
        """Connect across Qt bindings with different signal overloads."""

        if self._tryConnectSignal(self.currentIndexChanged):
            return

        try:
            current_text_changed = self.currentTextChanged
        except AttributeError:
            raise RuntimeError(
                "No compatible QComboBox index-change signal found") from None

        if not self._tryConnectSignal(current_text_changed):
            raise RuntimeError("No compatible QComboBox index-change signal found")

    def _tryConnectSignal(self, signal: _QtSignal) -> bool:
        for signature in (int, "int", "QString", str):
            try:
                signal[signature].connect(self.handleChanged)
                return True
            except (AttributeError, IndexError, KeyError, TypeError):
                pass

        try:
            signal.connect(self.handleChanged)
            return True
    ...
    def handleChanged(self, _index: Optional[Union[int, str]] = None) -> None:
        if self.firstUpdate:
