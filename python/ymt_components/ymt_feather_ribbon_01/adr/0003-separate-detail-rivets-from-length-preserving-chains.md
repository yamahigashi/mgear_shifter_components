...
- Curl-to-detail rotation offsets are scaled by animator-facing column multiplier attributes after curl-neighborhood blending and before the local curl offset is applied. Initial column multiplier values are stored on the guide as `detailCurlRotMults`; rig attributes expose a bounded channel-box slider range.
