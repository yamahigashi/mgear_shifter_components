...
class Component(component.Main):
    ...
    def addObjects(self):
        ...
        roll_t = transform.getTransformLookingAt(self.guide.apos[0],
                                                 self.guide.apos[1],
                                                 self.guide.apos[4] - self.guide.apos[3],
                                                 "yz",
                                                 False)
        roll_t = transform.setMatrixPosition(roll_t, self.guide.apos[0])
        self.roll1_npo = primitive.addTransform(
            self.root,
            self.getName("roll1_npo"),
            roll_t)
        self.roll1_ctl = self.addCtl(self.roll1_npo,
                                     "roll1_ctl",
                                     roll_t,
                                     self.color_ik,
                                     "crossarrow",
        ...
        self.roll2_npo = primitive.addTransform(
            self.ik2b_blend,
            self.getName("roll2_npo"),
            t_align)

        self.roll2_ctl = self.addCtl(self.roll2_npo,
                                     "roll2_ctl",
                                     t_align,
                                     self.color_ik,
                                     "crossarrow",
        ...
        attribute.lockAttribute(
            self.roll2_ctl, ["tx", "ty", "tz", "sx", "sy", "sz", "v"])
    ...
    def addOperators(self):
        ...
        # stable spring solver doble rotation
        pm.pointConstraint(self.root_ctl, self.chain2bones[0], mo=True)

        parentc_node = pm.parentConstraint(
            self.ik2b_ikCtl_ref, self.ik2b_bone_ref, self.ik2b_blend, mo=True)
        ...
        pm.parentConstraint(self.chain3bones[0], self.roll1_npo, mo=True)
