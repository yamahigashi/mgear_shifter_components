+ YamahigashiShifterComponents 0.1 .
PATH +:= ./bin
PYTHONPATH +:= ./python
MGEAR_SHIFTER_COMPONENT_PATH +:= python/ymt_components
MGEAR_SYNOPTIC_PATH +:= python/ymt_synoptics
MGEAR_SHIFTER_CUSTOMSTEP_PATH +:= python/ymt_customsteps

+ MAYAVERSION:2019 PLATFORM:win64 ymtshifterplugins 3.0 ./platforms/win64_2019
+ MAYAVERSION:2020 PLATFORM:win64 ymtshifterplugins 3.0 ./platforms/win64_2020
+ MAYAVERSION:2022 PLATFORM:win64 ymtshifterplugins 3.0 ./platforms/win64_2022
+ MAYAVERSION:2023 PLATFORM:win64 ymtshifterplugins 3.0 ./platforms/win64_2023
+ MAYAVERSION:2024 PLATFORM:win64 ymtshifterplugins 3.0 ./platforms/win64_2024
+ MAYAVERSION:2025 PLATFORM:win64 ymtshifterplugins 3.0 ./platforms/win64_2025
